...
@bot.tree.command(name="emsc", description="Türkiye'deki son 10 depremi listeler (EMSC)")
async def slash_emsc(interaction: discord.Interaction):
    await interaction.response.defer()
    try:
        async with aiohttp.ClientSession() as session:
            url = "https://www.seismicportal.eu/fdsnws/event/1/query?limit=50&format=json"
            async with session.get(url) as response:
                ...
                turkey_quakes = [
                    ...
                    if "TURKEY" in event["properties"]["flynn_region"].upper()
                ][:10]

                if not turkey_quakes:
                    await interaction.followup.send("❌ Son 50 deprem içinde Türkiye'de deprem bulunamadı.")
                    return

                for event in turkey_quakes:
                    ...
                    embed = discord.Embed(
                        ...
                        description=props["flynn_region"],
                        color=discord.Color.orange()
                    )
                    embed.add_field(name="💥 Büyüklük", value=props["mag"])
        ...
        print(f"[/quakes HATA] {e}")
        await interaction.followup.send("❌ EMSC verisi alınırken hata oluştu.")
